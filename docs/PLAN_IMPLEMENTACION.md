...
> **Estado (2026-07-30): 🟡 EN PROGRESO — fases 1-2 de 6 hechas (v2.44.0).** Plan de 6 fases
...
> equivalentes nativos del bridge.
> **Fase 2 — registro de recetas + primera receta:** `IReceta`/`RecetasRegistro`
> (`src/Recetas.cs`), botón receta = `# lang: receta` + JSON, primera receta `sql_tokens`
> (delega a `ctx.EjecutarSql`, ya probado). Cableado en ribbon y `BrosLMV.Runner`.
> **Probado en vivo contra `ComercialSP`** (éxito + error), agregado como **caso 8
> permanente del arnés de humo**. A propósito NO cableado en la Consola (las recetas no se
> editan a mano). **Pendiente real:** confirmar el render dentro de CONTPAQi de verdad
> (compila, pero es UI de Windows Forms sin forma de verificar visualmente sin abrir la
> Consola). **Faltan las fases 3-6** (almacén de estructuras, receta estrella, pasos
> encadenados, modo asistente) — ver `CHANGELOG.md` v2.43.0/v2.44.0 para el detalle completo.
