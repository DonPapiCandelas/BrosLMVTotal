...
# timeout: 1800
#
# PLANTILLA: Requisición de Compra — versión Forms (Python, WinForms real via pythonnet)
# La MISMA ventana que PLANTILLA_REQUISICION_FORMS_CSHARP.ctx (misma Solicitud de Compra,
# módulo 1040), pero orquestada 100% desde Python. Un usuario que no sepa C# puede
# copiar este patrón para construir sus propias ventanas.
#
# Nota real (2026-07-31): usa frm.ShowDialog() (modal, bloqueante) -- a diferencia de la
# versión C#, que usa frm.Show() (modeless, ver MANUAL.md §10). No se probó headless contra
# el sandbox por el mismo motivo que ctx.form(): ShowDialog() bloquea esperando a que un
# humano cierre la ventana, no hay forma de automatizarlo sin herramientas de UI automation
# que este proyecto no tiene. Se revisó el código (sintaxis válida, mismo patrón
# NuevoDocumento→AgregarArticulo→RecalcCompleto→Save ya probado en las otras 4 variantes)
# pero la interacción real de la ventana solo se puede confirmar abriéndola en Comercial.
#
