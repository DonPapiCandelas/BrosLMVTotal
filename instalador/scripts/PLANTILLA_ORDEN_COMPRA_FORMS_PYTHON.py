...
# timeout: 1800
#
# PLANTILLA: Orden de Compra -- version Forms (Python, WinForms real via pythonnet)
# La MISMA ventana que PLANTILLA_ORDEN_COMPRA_FORMS_CSHARP.ctx (Orden de Compra,
...
#
# Diferencia frente a Requisicion (PLANTILLA_REQUISICION_FORMS_PYTHON.py, modulo 1040):
# una Orden de Compra SI compromete un PRECIO por partida (es un acuerdo real con el
# proveedor) y captura la FECHA DE ENTREGA esperada.
#
# CORREGIDO (2026-07-31): esta plantilla decia "no se llama AffectStockNEW" -- INCORRECTO,
# ver la nota completa en PLANTILLA_ORDEN_COMPRA_FORMS_CSHARP.ctx (confirmado contra el
# sandbox real: orgProductKardex SI recibe una fila con Quantity=0, "compromete sin
# mover"). MANUAL.md Seccion 7.5 ya documentaba esto bien; el bug estaba solo aqui.
...
def crear_orden_compra(nueva):
    ...
    try:
        ...
        # SI se llama AffectStockNEW aqui (corregido, ver nota al inicio del archivo) --
        # para una OC deja el kardex con Quantity=0 ("compromete sin mover"), no lo salta.
        ctx.erp.AffectStockNEW(doc)
        if ctx.erp.LastError():
            raise Exception("AffectStockNEW: " + str(ctx.erp.LastError()))
        ctx.erp.Save(doc)
        if ctx.erp.LastError():
