# Caso de humo #28: valida PLANTILLA_REQUISICION_FORMS_SQL_PURO_CSHARP.ctx -- especificamente
# el cambio de v2.73.0 (docs/INVESTIGACION_XENGINE_SQL_PURO.md): tras el INSERT directo, el
# .ctx real llama ctx.erp.RecalcCompleto(doc) + ctx.erp.UpdateStatusDelivery(doc) en vez de
# calcular nada a mano. Ninguna de las 6 variantes de esta plantilla tenia caso de humo.
#
# LIMITE HONESTO (mismo que 25_adjuntos_documento.ps1): la plantilla real es una ventana
# WinForms modeless con buscador de proveedor/producto y un grid de partidas -- no
# automatizable headless con las herramientas de este proyecto. Este caso corre un script
# companion (28_requisicion_forms_sql_puro_csharp.codigo.cs) que reproduce LITERALMENTE el
# batch de T-SQL de CrearRequisicionSqlPuro() para una sola partida con parametros fijos,
# seguido de la MISMA secuencia de recalculo con ctx.erp que usa el .ctx real. El resultado se
# compara CAMPO POR CAMPO contra un documento nativo real (ctx.erp.NuevoDocumento/
# AgregarArticulo/RecalcCompleto/Save) -- si el recalculo via XEngine dejara algo distinto de
# lo que produce el camino 100% nativo, este caso lo atrapa.
param(
...
# 1) Documento de referencia NATIVO (ctx.erp) -- mismo producto/cantidad/proveedor que el
#    companion SQL puro usa por default.
$codigoNativo = @'
// job: safe-offline
...
return "doc=" + doc;
'@
if (-not (Upsert-Boton "HUMO28_REFERENCIA_NATIVA" "Humo 28 - referencia nativa" $codigoNativo)) { exit 1 }
$salidaRef = & $RunnerExe --appkey HUMO28_REFERENCIA_NATIVA --bd $Database 2>&1
Borrar-Boton "HUMO28_REFERENCIA_NATIVA"
...
$docNativo = Exec-Q "SELECT MAX(DocumentID) FROM docDocument WHERE ModuleID=1040"

# 2) Documento con el companion (misma logica que CrearRequisicionSqlPuro() del .ctx real).
$codigoCompanion = Get-Content (Join-Path $PSScriptRoot "28_requisicion_forms_sql_puro_csharp.codigo.cs") -Raw
if (-not (Upsert-Boton "HUMO28_SQL_PURO_CSHARP" "Humo 28 - Forms SQL puro (companion)" $codigoCompanion)) { exit 1 }
$salidaPuro = & $RunnerExe --appkey HUMO28_SQL_PURO_CSHARP --bd $Database 2>&1
Borrar-Boton "HUMO28_SQL_PURO_CSHARP"
...
$docSqlPuro = Exec-Q "SELECT MAX(DocumentID) FROM docDocument WHERE ModuleID=1040"
...
Write-Host "  [OK] El recalculo via ctx.erp/XEngine (companion Forms SQL puro) coincide campo por campo con el camino 100% nativo."
exit 0
