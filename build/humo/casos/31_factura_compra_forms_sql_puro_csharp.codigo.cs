// job: safe-offline
// Companion headless de PLANTILLA_FACTURA_COMPRA_FORMS_SQL_PURO_CSHARP.ctx para el caso de
// humo #31. NO es la plantilla real (ventana WinForms modeless que exige seleccionar 1+
// Ordenes de Compra en un grid nativo via ctx.GetSelectedIds() -- no automatizable headless,
// mismo limite documentado en 25_adjuntos_documento.codigo.cs). Reproduce LITERALMENTE el
// batch de T-SQL que arma CrearFacturaSqlPuro() en el .ctx real para UNA sola partida de UNA
// sola OC de origen, con parametros fijos -- los totales de cabecera se dejan en
// 0/placeholder (igual que el .ctx real), seguido EXACTAMENTE de la misma secuencia de
// recalculo con XEngine que el .ctx real ejecuta tras el COMMIT: ctx.erp.RecalcCompleto(doc)
// (SIN AffectStockNEW/UpdateStatusDelivery -- la Factura no afecta inventario) + la agenda de
// pago con montos reales leyendo el Total YA recalculado -- el cambio de v2.73.0 que esta
// plantilla capturaba sin ninguna prueba automatizada.
//
// Recibe --sourceOC / --sourceItemId como parametros del boton (el .ps1 los sustituye antes
// de registrar el boton, mismo patron que usa 30_recepcion_compra_forms_sql_puro_csharp).

int sourceOC = __SOURCE_OC__;
int sourceItemId = __SOURCE_ITEM_ID__;
int productoID = 1, taxTypeId = 5, condId = 4;
double cantidad = 10, precio = 100, descPerc = 0;
var ci = System.Globalization.CultureInfo.InvariantCulture;

int be = Convert.ToInt32(ctx.Scalar("SELECT BusinessEntityID FROM docDocument WHERE DocumentID=" + sourceOC));
int dep = Convert.ToInt32(ctx.Scalar("SELECT DepotID FROM docDocument WHERE DocumentID=" + sourceOC));
...
VALUES (
    152, 5, 2, 1, " + be + @",
    " + dep + @", 0, '', CAST(@folio AS NVARCHAR), GETDATE(), GETDATE(), GETDATE(), GETDATE(), GETDATE(), GETDATE(),
    3, 3, 1, " + condId + @", @ahora, 1, 1,
    0, 0, 0, 0, 0, N'PENDIENTE', 0, 0, 3, 0,
    N'', @ahora, 0, 0
);
...
SELECT
    ...
    '', " + taxTypeId + @", 0.16, " + precio.ToString(ci) + @", 0, " + descPerc.ToString(ci) + @", " + importe.ToString(ci) + @", 1, 1,
...
// RecalcCompleto (SIN AffectStockNEW/UpdateStatusDelivery -- la Factura no afecta inventario).
ctx.erp.RecalcCompleto(doc);
if (!string.IsNullOrEmpty(ctx.erp.LastError)) throw new Exception("RecalcCompleto: " + ctx.erp.LastError);

// Agenda de pago con montos reales, leyendo el Total YA recalculado por el motor (mismo
// patron que RegenerarPaymentAgenda() en la variante ctx.erp) -- reparto 50%/50% para
// PaymentTermID=4.
...
ctx.NonQuery("INSERT INTO docDocumentPaymentAgenda (DocumentID, DatePayment, TotalPerc, Amount, PartialityNumber, CreatedOn, CreatedBy) VALUES (" + doc + ", GETDATE(), 50, " + Math.Round(total * 0.5, 2).ToString(ci) + ", 1, GETDATE(), 0)");
ctx.NonQuery("INSERT INTO docDocumentPaymentAgenda (DocumentID, DatePayment, TotalPerc, Amount, PartialityNumber, CreatedOn, CreatedBy) VALUES (" + doc + ", DATEADD(MONTH,3,GETDATE()), 50, " + Math.Round(total * 0.5, 2).ToString(ci) + ", 2, GETDATE(), 0)");
