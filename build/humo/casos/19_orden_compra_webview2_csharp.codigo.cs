...
// timeout: 30
// lang: csharp
// CASO DE HUMO #19: copia de PLANTILLA_ORDEN_COMPRA_WEBVIEW2_CSHARP.ctx con auto-click
// inyectado (sin ctx.Msg -- usa throw/return para poder correr sin humano, ver
// MANUAL.md sobre limitaciones headless de ventanas bloqueantes).
...
int depotDefault = almacenes.Count > 0 ? Convert.ToInt32(almacenes[0]["DepotID"]) : 0;
string serieDefault = "", folioDefault = "";
try {
    serieDefault = ctx.erp.GetFolioPrefix(183, depotDefault) ?? "";
    folioDefault = ctx.erp.GetNextFolio(183, serieDefault, depotDefault) ?? "";
} catch { }
...
string idsProductos = productos.Count > 0 ? string.Join(",", productos.Select(p => p["ProductID"])) : "0";

var existRows = ctx.Query(@"
    SELECT k.ProductID, d.DepotName, SUM(k.Quantity) AS Qty
    FROM orgProductKardex k INNER JOIN orgDepot d ON d.DepotID=k.DepotID
    WHERE k.Cancelled=0 AND d.DeletedOn IS NULL AND k.ProductID IN (" + idsProductos + @")
    GROUP BY k.ProductID, d.DepotName ORDER BY d.DepotName");
var listaRows = ctx.Query(@"
    ...
    WHERE pl.DeletedOn IS NULL AND ppl.ProductID IN (" + idsProductos + @")
    ORDER BY pl.PriceListName");
var provRows = ctx.Query(@"
    ...
    WHERE ps.ProductID IN (" + idsProductos + @")
...
var monedaRateJson = "{" + string.Join(",", monedas.Select(m => "\"" + m["CurrencyID"] + "\":" + N(m["Rate"]))) + "}";
var monedaPalabraJson = "{" + string.Join(",", monedas.Select(m => {
    string cur = Convert.ToString(m["Currency"] ?? "").ToUpperInvariant();
    string palabra = cur.Contains("PESO") ? "PESOS" : (cur.Contains("DOLAR") || cur.Contains("DÓLAR")) ? "DÓLARES"
        : cur.Contains("EURO") ? "EUROS" : (cur.Length > 0 ? cur + "S" : "PESOS");
    return "\"" + m["CurrencyID"] + "\":\"" + palabra + "\"";
})) + "}";
...
var existPorProducto = existRows.GroupBy(r => Convert.ToInt32(r["ProductID"]))
    .ToDictionary(g => g.Key, g => g.ToList());
var listaPorProducto = listaRows.GroupBy(r => Convert.ToInt32(r["ProductID"]))
    .ToDictionary(g => g.Key, g => g.ToList());
var provPorProducto = provRows.GroupBy(r => Convert.ToInt32(r["ProductID"]))
    .ToDictionary(g => g.Key, g => g.ToList());

var productosJson = "[" + string.Join(",", productos.Select(p => {
    int pid = Convert.ToInt32(p["ProductID"]);
    string ex = existPorProducto.ContainsKey(pid)
        ? string.Join(",", existPorProducto[pid].Select(e => "{\"almacen\":\"" + JsonEsc(Convert.ToString(e["DepotName"])) + "\",\"qty\":" + N(e["Qty"]) + "}"))
        : "";
    string ls = listaPorProducto.ContainsKey(pid)
        ? string.Join(",", listaPorProducto[pid].Select(e => "{\"lista\":\"" + JsonEsc(Convert.ToString(e["PriceListName"])) + "\",\"precio\":" + N(e["Price"]) + "}"))
        : "";
    string pv = provPorProducto.ContainsKey(pid)
        ? string.Join(",", provPorProducto[pid].Select(e => "{\"proveedor\":\"" + JsonEsc(Convert.ToString(e["OfficialName"])) + "\",\"ref\":\"" + JsonEsc(Convert.ToString(e["RefSupplier"])) + "\",\"costo\":" + N(e["CostPrice"]) + "}"))
        : "";
    return "{\"id\":" + pid +
        ",\"key\":\"" + JsonEsc(Convert.ToString(p["ProductKey"])) + "\"" +
        ",\"nombre\":\"" + JsonEsc(Convert.ToString(p["ProductName"])) + "\"" +
        ",\"unidad\":\"" + JsonEsc(Convert.ToString(p["Unit"])) + "\"" +
        ",\"descripcion\":\"" + JsonEsc(Convert.ToString(p["ProductDescription"])) + "\"" +
        ",\"unitSale\":\"" + JsonEsc(Convert.ToString(p["UnitSale"])) + "\"" +
        ",\"unitBuy\":\"" + JsonEsc(Convert.ToString(p["UnitBuy"])) + "\"" +
        ",\"costo\":" + N(p["CostPrice"]) +
        ",\"precioLista\":" + N(p["PriceList"]) +
        ",\"taxTypeId\":" + Convert.ToInt32(p["TaxTypeID"]) +
        ",\"cat1\":\"" + JsonEsc(Convert.ToString(p["Category1"])) + "\"" +
        ",\"cat2\":\"" + JsonEsc(Convert.ToString(p["Category2"])) + "\"" +
        ",\"cat3\":\"" + JsonEsc(Convert.ToString(p["Category3"])) + "\"" +
        ",\"cat4\":\"" + JsonEsc(Convert.ToString(p["Category4"])) + "\"" +
        ",\"exist\":[" + ex + "]" +
        ",\"listas\":[" + ls + "]" +
        ",\"proveedores\":[" + pv + "]}";
})) + "]";
...
<body>
  ...
  <div class=""card"">
    ...
    <div class=""row"">
      ...
      <div><label>Moneda</label><select id=""moneda"" onchange=""render()"">" + opcionesMoneda + @"</select></div>
    ...
    <div class=""row"">
      ...
      <div><label>Serie</label><input id=""serie"" value=""" + JsonEsc(serieDefault) + @""" readonly></div>
      <div><label>Folio</label><input id=""folio"" value=""" + JsonEsc(folioDefault) + @""" readonly></div>
    </div>
...
<script>
  ...
  const MONEDA_RATE = " + monedaRateJson + @";
  const MONEDA_PALABRA = " + monedaPalabraJson + @";
  let partidas = [];
  ...
  selProd.selectedIndex = 0; preseleccionarImpuesto();
...
// ═══════════════════ MOSTRAR Y RECIBIR ═══════════════════
var r = ctx.ShowHtmlFormulario(html, "Nueva Orden de Compra (WebView2)", 1020, 820);
...
string fecha = r.ContainsKey("fecha") ? Convert.ToString(r["fecha"]) : "";
...
double rate = 1;
try { var rr = ctx.Query("SELECT Rate FROM vwLBSCurrencyList WHERE CurrencyID=" + moneda); if (rr.Count > 0) rate = Convert.ToDouble(rr[0]["Rate"] ?? 1); } catch { }

int doc = ctx.erp.NuevoDocumento(183, depot, be);
string fechaEntregaSql = string.IsNullOrEmpty(entrega) ? "GETDATE()" : "'" + entrega.Replace("-", "") + "'";
string fechaDocSql = string.IsNullOrEmpty(fecha) ? "GETDATE()" : "'" + fecha.Replace("-", "") + "'";
ctx.NonQuery("UPDATE docDocument SET DepotIDFrom=0, CurrencyID=" + moneda + ", Rate=" + rate.ToString(CultureInfo.InvariantCulture) +
             ", PaymentTermID=" + condicion +
             ", DateDocument=" + fechaDocSql +
             ", DateDelivery=" + fechaEntregaSql + ", DateDocDelivery=" + fechaEntregaSql +
...
foreach (Dictionary<string, object> p in partidasRecibidas)
{
    ...
    int taxTypeId = p.ContainsKey("taxTypeId") ? Convert.ToInt32(p["taxTypeId"]) : -1;
    double descuentoPerc = p.ContainsKey("descuentoPerc") ? Convert.ToDouble(p["descuentoPerc"]) : 0;
    ctx.erp.AgregarArticulo(doc, productId, cantidad, precio, -1, taxTypeId, descuentoPerc);
    ctx.NonQuery(
        "IF NOT EXISTS (SELECT 1 FROM orgProductSupplier WHERE ProductID=" + productId + " AND SupplierID=" + supplierID + ") " +
        "INSERT INTO orgProductSupplier (ProductID, SupplierID, CostPrice, CurrencyID, OrderNumber) VALUES (" + productId + ", " + supplierID + ", " + precio.ToString(CultureInfo.InvariantCulture) + ", " + moneda + ", 0) " +
        "ELSE UPDATE orgProductSupplier SET CostPrice=" + precio.ToString(CultureInfo.InvariantCulture) + " WHERE ProductID=" + productId + " AND SupplierID=" + supplierID);
}
